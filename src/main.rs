...
fn main() -> Result<(), String> {
    ...
    let load_and_run = |input: &str| {
        let path = |file_name: &str| -> PathBuf {
            let path_name = format!("./inputs/{}/{}/{}.txt", args.day, input, file_name);
    ...
    match &args.input {
        ...
        None => {
            // TODO: Make input optional. When not specified:
            // Enumerate all folders in the day's inputs
            // Run the example first
            // Run anything not called real next, alphabetically
            // Run real last
        },
    };
